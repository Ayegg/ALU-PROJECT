...
// =================================================================
// MODUL: TESTBENCH (Mediul de Simulare)
// Spre deosebire de modulele anterioare, acest cod NU se transforma 
// in hardware (piese fizice pe placa). Este doar un program software 
// care simuleaza apasarea butoanelor si citirea ecranului.
// =================================================================
module tb_alu();

    // In testbench, INTARIRILE modulului testat trebuie declarate ca 'reg' 
    // pentru ca noi, din simulare, le vom da valori (ca si cum am apasa pe butoane).
    reg clk, rst, start;
    reg [7:0] A, B;
    reg [3:0] opcode;
    
    // IESIRILE modulului testat trebuie declarate ca 'wire' 
    // pentru ca doar le observam (sunt conduse de modul, nu de noi).
    wire [7:0] result;
    wire done, Z, N, V;
    ...
    initial begin
        // Resetam sistemul pentru a curata orice "gunoi" din memorie
        rst = 1; start = 0; A = 0; B = 0; opcode = 0;
        #10 rst = 0; // Asteptam 10ns si oprim reset-ul

        // 1. ADUNARE (ADD): 15 + 10 = 25
        ...
        // 2. SCADERE (SUB): 20 - 5 = 15
        #10 A = 8'd20; B = 8'd5; opcode = 4'b0001;
        #1 $display("2. SUB: A=%d, B=%d, Result=%d", A, B, result);

        // 3. INMULTIRE (MUL): 6 * 4 = 24 (OPERATIE SECVENTIALA)
        // Aici dam un impuls scurt pe butonul de 'start' (ca un click de mouse).
        ...
        // pana cand modulul ridica steagul 'done' (done == 1).
        wait(done == 1'b1);
        #1 $display("3. MUL: A=%d, B=%d, Result=%d", A, B, result);

        // 4. IMPARTIRE (DIV): 25 / 5 = 5 (OPERATIE SECVENTIALA)
