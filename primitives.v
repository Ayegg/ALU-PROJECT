...
// REGISTRU PE 8 BITI - Memoreaza un numar intreg
module reg_8bit (
    input clk, rst, en,
    input [7:0] d,
    output [7:0] q
...
// SUMATOR COMPLET (Full Adder) DE 1 BIT
module full_adder(
    input a, b, cin,
    output sum, cout
);
    // Implementare cu porti logice pentru calculul matematic A + B + CarryIn
    wire w1, w2, w3;
...
// MUX 2:1 pe 8 biti (Alege intre doua "magistrale" de 8 fire)
module mux_2to1_8bit (input [7:0] d0, d1, input sel, output [7:0] y);
    genvar i;
    generate
        for (i=0; i<8; i=i+1) begin : mux_inst
            // Folosim 8 multiplexoare de 1 bit asezate in paralel, 
...
module mux_8to1_8bit (
   ...
   // Se aleg 4 din 8 date initiale pe baza lui sel[0]
    ...
    // Reducem cele 4 rezultate la doar 2 pe baza lui sel[1]
    mux_2to1_8bit m4 (.d0(w0), .d1(w1), .sel(sel[1]), .y(w4));
    mux_2to1_8bit m5 (.d0(w2), .d1(w3), .sel(sel[1]), .y(w5));
    
    // Alegem rezultatul final pe baza ultimului bit, sel[2]
